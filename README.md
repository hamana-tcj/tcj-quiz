...
#### 方法1: GitHub Actionsのログを確認（推奨・簡単）

GitHub Actionsのログでも十分に情報が得られます。
...
3. **ワークフローを選択**
   - 左側のメニューから「**Sync kintone users to Supabase**」を選択

4. **実行履歴を確認**
   - メイン画面に実行履歴が表示されます
   - 各実行をクリックすると、詳細なログが表示されます

5. **ログの見方**
   - 「**sync**」ジョブをクリック
   - 各ステップのログを展開して確認
   - `curl`コマンドのレスポンスに処理結果が含まれます

**確認すべき情報:**
- `totalCreated`: 作成されたユーザー数
- `totalSkipped`: スキップされたユーザー数
- `batches`: 各バッチの処理結果
- `stoppedReason`: 処理が中断された理由（timeoutなど）

#### 方法2: Vercelのログを確認（詳細なデバッグ情報）

Vercelのログには、より詳細な`console.log()`の出力が含まれます。
...
2. **デプロイメントを確認**
   - プロジェクトページの上部で「**Deployments**」タブをクリック
   - 最新のデプロイメントを選択

3. **デプロイメントの詳細を表示**
   - デプロイメントをクリックすると、詳細ページが開きます
   - 下部にスクロールして「**Runtime Logs**」セクションを確認

4. **Functionsのログを確認（Vercel Pro以上）**
   - プロジェクトページの左側メニューから「**Logs**」を選択（Proプラン以上）
   - または、デプロイメント詳細ページで「**Functions**」タブを開く
...
**注意:**
- Vercelの無料プランでは、ログの保存期間が制限されている場合があります
- 「Logs」メニューはProプラン以上で利用可能です
- 無料プランの場合は、「Deployments」→「Runtime Logs」で確認できます
